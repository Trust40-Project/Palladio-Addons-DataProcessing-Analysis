...
public class ConfigurationTab extends AbstractLaunchConfigurationTab {
	...
	public ConfigurationTab() {
		IProverManager activator = Activator.getInstance().getProverManagerInstance();
		if (activator != null) {
			proversMap = activator.getProvers();
		} else {
			// TODO empty Map?
		}
	}
	...
	public void setDefaults(ILaunchConfigurationWorkingCopy configuration) {
		...
		configuration.setAttribute(Constants.ANALYSIS_GOAL_LABEL.getConstant(), "default"); // Saving the ID of the
																							// ProverManager
		configuration.setAttribute(Constants.PROLOG_INTERPRETER_LABEL.getConstant(), ""); // Saving name of QueryInput
	}
	...
	public void initializeFrom(ILaunchConfiguration configuration) {
		...
		try {
			...
			prologCombo.select(prologCombo.indexOf(prologConfig));
	...
	public void performApply(ILaunchConfigurationWorkingCopy configuration) {
		...
		for (Map.Entry<ProverInformation, IProverFactory> entry : proversMap.entrySet()) {
			if ((entry.getKey().getName()).equals(analysisName)) {
		...
		configuration.setAttribute(Constants.PROLOG_INTERPRETER_LABEL.getConstant(),
				prologCombo.getItem(prologCombo.getSelectionIndex()));
	}
	...
	public void createControl(Composite parent) {
		...
		for (Map.Entry<ProverInformation, IProverFactory> entry : proversMap.entrySet()) {
			analysisCombo.add(entry.getKey().getName());
		}
		...
		for (QueryInput query : QueryInput.values()) {
			prologCombo.add(query.getName());
		}
